...
// Usada para o cadastro manual para evitar duplicatas simples.
...
// Usada para importação de planilhas (Excel) que não possuem IDs.
const upsertAlunoByNomeTurma = (db, aluno) => {
    const nomeNormalizado = normalizeString(aluno.nome);
    const turmaNormalizada = normalizeString(aluno.turma);
    ...
    let existingId = null;
    if (selectStmt.step()) {
        existingId = selectStmt.getAsObject().id;
    }
    selectStmt.free();

    if (existingId) {
        const updateStmt = db.prepare("UPDATE alunos SET nome = ?, turma = ? WHERE id = ?");
        updateStmt.run([aluno.nome.trim(), aluno.turma.trim(), existingId]);
        updateStmt.free();
        return { action: 'updated' };
    } else {
        const newId = crypto.randomUUID();
        const insertStmt = db.prepare(`INSERT INTO alunos (id, nome, turma, codigo_barra, nome_normalizado, turma_normalizada) VALUES (?, ?, ?, ?, ?, ?)`);
        insertStmt.run([newId, aluno.nome.trim(), aluno.turma.trim(), gerarCodigoBarrasValido(), nomeNormalizado, turmaNormalizada]);
        insertStmt.free();
        return { action: 'inserted' };
    }
};

// **NOVA FUNÇÃO** - Usada exclusivamente para importação de bancos de dados (.db) para preservar IDs.
const upsertAlunoById = (db, aluno) => {
    ...
    if (existe) {
        const updateStmt = db.prepare(`
            UPDATE alunos 
            SET nome = ?, turma = ?, codigo_barra = ?, foto = ?, nome_normalizado = ?, turma_normalizada = ? 
            WHERE id = ?
        `);
        updateStmt.run([aluno.nome, aluno.turma, aluno.codigo_barra, aluno.foto, nomeNormalizado, turmaNormalizada, aluno.id]);
        updateStmt.free();
        return { action: 'updated' };
    } else {
        const insertStmt = db.prepare(`
            INSERT INTO alunos (id, nome, turma, codigo_barra, foto, nome_normalizado, turma_normalizada) 
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `);
        // Garante que um código de barras exista se o do backup for nulo
        const codigoBarras = aluno.codigo_barra || gerarCodigoBarrasValido();
        insertStmt.run([aluno.id, aluno.nome, aluno.turma, codigoBarras, aluno.foto, nomeNormalizado, turmaNormalizada]);
        insertStmt.free();
        return { action: 'inserted' };
    }
};
...
// Função para Excel continua usando a lógica de nome/turma
    ...
    reader.onload = e => {
        try {
            ...
            sheetData.forEach(aluno => {
                ...
                if (nome && turma) {
                    const result = upsertAlunoByNomeTurma(db, { nome: String(nome), turma: String(turma) });
                    if (result?.action === 'inserted') importados++;
                    if (result?.action === 'updated') atualizados++;
                }
            });
            db.exec("COMMIT;");
            resolve({ importados, atualizados });
        } catch (err) {
            db.exec("ROLLBACK;");
            reject(err);
        }
...
// **FUNÇÃO MODIFICADA** - Agora usa a lógica correta para importar de .db
const importFromSQLite = async (file, db) => {
    ...
    // A consulta agora pega todos os dados, não apenas nome e turma
    ...
    if (result.length > 0 && result[0].values.length > 0) {
        // Mapeia os nomes das colunas para seus índices para fácil acesso
        const columns = result[0].columns;
        const colMap = {};
        columns.forEach((col, index) => { colMap[col] = index; });

        db.exec("BEGIN TRANSACTION;");
        try {
            result[0].values.forEach(row => {
                // Cria um objeto aluno completo com todos os dados da linha
                ...
                // Chama a nova função que preserva o ID
                const res = upsertAlunoById(db, alunoFromImport);
...
// =================================================================================
// FUNÇÕES PARA PERSISTIR E CARREGAR A BASE DE DADOS
// =================================================================================
...
const getSchoolsFromIDB = async () => {
    if (!state.idb) await openAndSetIDB();
    return new Promise((resolve, reject) => {
        const tx = state.idb.transaction(IDB_CONFIG.storeName, 'readonly');
...
const loadSchool = async (school) => {
    try {
        if (!await verifyPermission(school.dbHandle) || (school.logoHandle && !await verifyPermission(school.logoHandle)) || (school.templateHandle && !await verifyPermission(school.templateHandle))) {
...
// =================================================================================
// EVENT LISTENERS E FUNÇÕES ADICIONAIS
// =================================================================================
...
const renderAlunosTable = () => {
    ...
    state.alunos.sort((a, b) => a.Nome.localeCompare(b.Nome)).forEach(aluno => {
        ...
        row.innerHTML = `<td data-field="nome">${aluno.Nome}</td><td data-field="turma">${aluno.Turma}</td><td>${aluno.Codigo_Barras}</td><td class="foto-cell">${fotoPreview}<label class="btn-upload-overlay">📷<input type="file" class="foto-upload-input" data-aluno-id="${aluno.id}" accept="image/*" style="display:none;"></label></td><td class="actions-cell"><button class="btn btn-small edit-btn" title="Editar"><span style="font-size:1.2em;">✏️</span></button><button class="btn btn-small btn-error delete-btn" title="Excluir"><span style="font-size:1.2em;">🗑️</span></button></td>`;
...
const handleImportFile = async file => {
    ...
    try {
        ...
        let mensagem = `${result.importados} novo(s) aluno(s) importado(s) e ${result.atualizados} aluno(s) atualizado(s) com sucesso!`;
...
tabelaAlunosBody.addEventListener('click', async e => {
    ...
    if (saveBtn) {
        ...
        if (!novoNome || !novaTurma) { alert("Nome e Turma não podem ser vazios."); return; }
        try {
    ...
    if (cancelBtn) {
        ...
        if (!row) return;
        renderAlunosTable(); // Simplesmente re-renderiza a tabela para cancelar
        return;
    }
});
...
addSchoolForm.addEventListener('submit', async e => {
    ...
    if (editingSchoolId) {
        ...
        if (!schoolName || !newSchoolFiles.template || !newSchoolFiles.db) {
            updateStatus(modalStatus, "Nome, template e base de dados são obrigatórios.", 'error');
            return modalStatus.style.display = 'block';
        }
...
changeSchoolBtn.addEventListener('click', async () => { showSchoolSelection(); await renderSchoolList(); });
...
selectLogoBtn.addEventListener('click', async () => { try { const [h] = await window.showOpenFilePicker({ types: [{ description: 'Imagens', accept: { 'image/*': ['.png', '.jpg', '.jpeg'] } }] }); newSchoolFiles.logo = h; logoFileNameSpan.textContent = h.name; } catch(e){} });
selectTemplateBtn.addEventListener('click', async () => { try { const [h] = await window.showOpenFilePicker({ types: [{ description: 'Imagens', accept: { 'image/*': ['.png', '.jpg', '.jpeg'] } }] }); newSchoolFiles.template = h; templateFileNameSpan.textContent = h.name; } catch(e){} });
selectDbBtn.addEventListener('click', async () => { try { const [h] = await window.showOpenFilePicker({ types: [{ description: 'Banco de Dados SQLite', accept: { 'application/x-sqlite3': ['.db', '.sqlite'] } }] }); newSchoolFiles.db = h; dbFileNameSpan.textContent = h.name; } catch(e){} });
createDbModalBtn.addEventListener('click', async () => { try { const h = await window.showSaveFilePicker({ suggestedName: 'nova_base_alunos.db', types: [{ description: 'Banco de Dados SQLite', accept: { 'application/x-sqlite3': ['.db', '.sqlite'] } }] }); newSchoolFiles.db = h; dbFileNameSpan.textContent = `(Nova) ${h.name}`; } catch(e){} });
...
listaCrachasAlunos.addEventListener('click', e => {
    if (e.target.tagName === 'LI') {
        ...
        if (state.selectedAlunoCracha) {
            gerarCracha(state.selectedAlunoCracha).then(() => { downloadCrachaBtn.disabled = false; renderCrachasList(); }).catch(() => { downloadCrachaBtn.disabled = true; });
        }
    }
});
downloadCrachaBtn.addEventListener('click', () => { if (state.selectedAlunoCracha) { const a = document.createElement('a'); a.download = `${state.selectedAlunoCracha.Nome.replace(/ /g, '_')}_cracha.png`; a.href = crachaCanvas.toDataURL('image/png'); a.click(); } });
downloadTodosCrachasBtn.addEventListener('click', async () => {
    ...
    for (let i = 0; i < state.alunos.length; i++) {
        ...
        } catch (err) { console.error(`Falha no crachá de ${aluno.Nome}`); }
    }
    updateStatus(zipStatus, 'Compactando...', 'warning');
    const content = await zip.generateAsync({ type: 'blob' });
    const a = document.createElement('a'); a.href = URL.createObjectURL(content); a.download = 'crachas.zip'; a.click(); URL.revokeObjectURL(a.href);
...
selectFolderBtn.addEventListener('click', async () => { try { const h = await window.showDirectoryPicker({mode: 'readwrite'}); state.directoryHandle = h; await saveDirectoryHandle(h); updateStatus(folderStatus, `Pasta selecionada e salva: "${h.name}".`, 'success'); } catch (err) { if (err.name !== 'AbortError') updateStatus(folderStatus, `Erro: ${err.message}`, 'error'); } });
...
importDropZone.addEventListener('drop', e => { e.preventDefault(); importDropZone.classList.remove('dragover'); if (e.dataTransfer.files.length > 0) handleImportFile(e.dataTransfer.files[0]); });
exportDbBtn.addEventListener('click', async () => { if (!state.db || !state.activeSchool) { alert('Nenhuma escola ativa.'); return; } if (await persistDatabase()) exportDatabase(state.db, state.activeSchool.name); });
...
tabelaCadastroManualBody.addEventListener('click', e => { if (e.target.classList.contains('delete-row-btn')) { e.target.closest('tr').remove(); } });
